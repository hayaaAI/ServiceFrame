...
namespace Hayaa.RemoteService.DataAccess
{
    ...
    {      
        ...
        {
            string sql = "insert AppConfig (AppConfigID,AppID,SolutionID,SolutionName,Version,ConfigContent,CreateTime) values(@AppConfigID,@AppID,@SolutionID,@SolutionName,@Version,@ConfigContent,@CreateTime)";
            return Update<AppConfig>(g_con,sql, info) ;
        }
		  internal static int update(AppConfig info)
        {
            string sql = "update AppConfig set AppConfigID=@AppConfigID,AppID=@AppID,SolutionID=@SolutionID,SolutionName=@SolutionName,Version=@Version,ConfigContent=@ConfigContent,CreateTime=@CreateTime where AppConfigID=@AppConfigID";
            return Update<AppConfig>(g_con,sql, info) ;
        }
